...
import { isItemDefinition, } from '../../manifest.types';
...
const generateRenderingParams = (component, rendering) => {
    ...
    if (!Array.isArray(component.params)) {
        throw chalk.red(`An instance of ${component.name} defined param(s) '${reducedParams
            .map((rp) => rp.name)
...
const generateChildrenFields = (children) => {
    ...
    result.forEach((child) => {
        ...
        if (child.fields) {
            child.fields = convertComponentDataToFields({
                data: child.fields,
                context: { item: child },
            });
        }
