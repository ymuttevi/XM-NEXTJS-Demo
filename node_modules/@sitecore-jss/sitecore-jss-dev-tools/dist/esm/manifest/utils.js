...
export const resolveFiles = ({ fileGlob, extensions, recursive, workingDirectory, }) => {
    let files = [];
...
export const importModules = ({ fileGlobs = [], workingDirectory = '', }) => {
    let resolved = [];
    fileGlobs.forEach((fileGlob) => {
        resolved = resolved.concat(resolveFiles({ fileGlob, extensions: ['.js'], recursive: true, workingDirectory }));
    });
    if (!resolved.length) {
