 ...
 */
export function setup(interactive, outputFile, initialData, configName = 'sitecore') {
    ...
    if (interactive) {
        configObject.deployUrl = getInteractiveData(configObject.deployUrl || urljoin(configObject.layoutServiceHost, defaultDeployUrl), interactive, 'host', 'Sitecore import service URL', '(usually same as hostname)', getValidation(/^https?:\/\/(.*)/), 'Invalid input. Must start with http(s)');
    }
    // API KEY
    ...
    console.log(`* Test your app in integrated mode by visiting ${chalk.green(configObject.layoutServiceHost)}`);
}
