...
const generateRenderings = ({ components, templates }) => components.map((component) => {
    ...
    const template = utils_1.findTemplateForComponent(component, templates);
...
exports.default = (args) => {
    ...
    const duplicateIds = utils_1.checkUnique(finalRenderings, (rendering) => rendering.id);
    if (duplicateIds.length > 0) {
        throw `The manifest defined duplicate rendering IDs: ${duplicateIds.join(',')}. This is not allowed.`;
    }
    const duplicateNames = utils_1.checkUnique(finalRenderings, (rendering) => rendering.name);
