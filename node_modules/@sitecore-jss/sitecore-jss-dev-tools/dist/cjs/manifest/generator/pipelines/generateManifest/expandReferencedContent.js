...
function expandReferencedContent(manifest, components) {
    ...
    const idMap = createIdMapping_1.createIdMapping(manifest);
    ...
    idMap.usages.reverse().forEach((idReference) => {
        ...
        traversal_1.traverseAllItems(refDeepClone.children, (item) => {
            delete item.id;
        });
        if (targetType === 'rendering') {
            ...
            if (expandedRef.template) {
                const component = utils_1.findComponentForTemplate(expandedRef.template, components);
